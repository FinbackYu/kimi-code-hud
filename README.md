...
| TPS / TTFT / Cache / thinking / goal / swarm | 增量解析 `~/.kimi-code/sessions/*/session_<id>/agents/main/wire.jsonl`（旧版为 `ses_<id>`，两者都兼容）的 `turn.prompt`、`step.end`、`config.update`、`goal.*` 与 `swarm_mode.*` 事件（byte offset 与当前回合 Cache 计数存 `~/.kimi-code-hud/metrics-<sessionId>.json`，每秒只读新增字节；旧状态仅做一次最多 1 MiB 的 Cache 回填） |
