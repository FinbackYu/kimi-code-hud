 ...
 * orchestration (swarm/tower) members never linger in the head count), and
 * except that a parked main drops out too — blocked inside the AgentSwarm
 * tool or waiting on tower workers, or a solo main blocked in a single Agent
 * tool call while its turn is still open — so its pre-block speed is never
 * summed into the fleet while it is not generating: with several active
