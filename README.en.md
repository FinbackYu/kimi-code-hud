...
| TPS / TTFT / Cache / thinking / goal / swarm | incremental parsing of `turn.prompt`, `step.end`, `config.update`, `goal.*` and `swarm_mode.*` events in `~/.kimi-code/sessions/*/session_<id>/agents/main/wire.jsonl` (legacy `ses_<id>` also supported) (byte offset and current-turn Cache counters persist in `~/.kimi-code-hud/metrics-<sessionId>.json`; only new bytes are read each second, with a one-time Cache restoration capped at 1 MiB for old state files) |
