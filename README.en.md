...
- Model/thinking prefers the wire (`profile.bind` plus every request's `llm.request`); without wire rows it is pinned by the per-session snapshot `~/.kimi-code-hud/thinking-<sessionId>.json`, and only then falls back to `config.toml` (writing the snapshot) — so another session running `/effort` never bleeds into this one;
