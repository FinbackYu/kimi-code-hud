...
| Segment | Source |
|---|---|
| model / branch | stdin snapshot + `git status --porcelain` resolved from PATH to an absolute executable outside the workspace (150ms timeout; no trusted `git` silently omits the dirty marker) |
...
- [Capabilities](CAPABILITIES.md): coverage of Kimi Code 0.36.0's line-1 slots, data sources, and readable-but-unrendered Cache/token/goal/task/Git information;
