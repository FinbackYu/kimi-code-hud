...
Cache is the token-weighted prompt-cache hit rate for the whole session: `Σ inputCacheRead / Σ (inputOther + inputCacheRead + inputCacheCreation)`, accumulated across turns over every main-agent model request (steps with incomplete usage fields are skipped). When a new turn has prompted but not yet contributed usage, the ratio stays visible dimmed instead of disappearing, so the line width no longer jumps; it brightens again once that turn's first valid `step.end` is counted. The segment is omitted only while the session has no data at all. Compact/normal show the percentage; full also shows cache-read/total-input tokens. No red/yellow/green quality threshold is applied.
...
**No Cache segment?** It stays omitted only while the session has no complete `step.end` usage yet. Once the first valid usage lands, the segment appears and never disappears again — a new prompt merely dims it until that turn contributes. After upgrades, a bounded restoration (at most 1 MiB of the wire tail) rebuilds the cumulative counters once.
