...
| Kimi Code host | Source/fixture baseline **2.1.0**; live TUI acceptance pending | Per-release contract audits with pinned upstream commits live in [CAPABILITIES.md](CAPABILITIES.md) |
...
- [Capabilities](CAPABILITIES.md): coverage of Kimi Code 2.1.0's line-1 slots, data sources, and readable-but-unrendered Cache/token/goal/task/Git information;
...
**No DeepSeek balance or Session Cost?** The active supported model must use a provider named `deepseek`, `type = "openai"`, and the official `https://api.deepseek.com` base (optionally `/v1`). On this prep branch, balance still requires a literal `api_key`; an env-only credential fails closed and hides the balance. `main` supports Kimi Code 2.0.1+ `api_key_env` through PR #46 (not yet in a new HUD release); this prep branch needs to sync from `main` before it supports that path ([KI-21](KNOWN_ISSUES.md#ki-21-provider-balance-does-not-resolve-api_key_env)). Before the first detached refresh finishes, neither balance nor cost has a loading placeholder (cost must first learn CNY or USD from the balance response). Once the complete usage ledger is ready, Session Cost can appear alone even when balance is unavailable if its currency is known. Run `node ~/.kimi-code/plugins/managed/kimi-code-hud/bin/kimi-hud.mjs --refresh-provider-usage deepseek` silently and then inspect `~/.kimi-code-hud/provider-usage/`. Compatible proxies are intentionally refused for both.
