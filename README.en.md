...
| TPS / TTFT | incremental parsing of `step.end` events in **all** `agents/*/wire.jsonl` of the session (main + every subagent; legacy `ses_<id>` also supported). Samples carry the event timestamp: only the freshest 5 within the last 10 minutes feed the median, so resume continuations, long idle gaps and compactions never mix in stale numbers; TTFT comes from the newest sample. While the model is generating (an `llm.request` with no later `step.end`, compaction requests excluded) the segment swaps TTFT for a live `gen Ns` ticker that updates every refresh (per-agent byte offsets persisted in `~/.kimi-code-hud/metrics-<sessionId>.json`; only new bytes are read each second) |
