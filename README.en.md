...
A zero-dependency custom status line (HUD) for **Kimi Code CLI** — shows model, git branch, context usage, generation speed (TPS/TTFT), current-turn cache hit rate and API quota in the TUI footer.
...
```
K3 thinking:high │ kimi-code-hud git:(main*) │ ⚡ 47 t/s · TTFT 1.3s │ Cache 92% │ 5h ███░░░░░░░ 31% ~2h18m │ wk ██░░░░░░░░ 25%
```
...
normal:  [manual] K3 thinking:high │ kimi-code-hud git:(main*) │ ⚡ 47 t/s · TTFT 1.3s │ Cache 92% │ 5h ███░░░░░░░ 31% ~2h18m │ wk ██░░░░░░░░ 25%
full:    [manual] K3 thinking:high │ kimi-code-hud git:(main*) │ Context ██████░░░░ 62% (159K/256K) │ ⚡ 47 t/s · TTFT 1.3s │ Cache 92% (86K/94K) │ 5h ███░░░░░░░ 31% ~2h18m │ wk ██░░░░░░░░ 25% ~3d2h │ v0.31.0
```
...
Cache is the token-weighted prompt-cache hit rate for the current user turn: `Σ inputCacheRead / Σ (inputOther + inputCacheRead + inputCacheCreation)`. It combines every main-agent model request triggered by one user message, including continuations after tool calls, and clears immediately on the next `turn.prompt`. The whole segment is omitted before the first valid `step.end`, when usage fields are incomplete, or when a bounded restoration of at most 1 MiB cannot find a complete turn boundary. Compact/normal show the percentage; full also shows cache-read/total-input tokens. No red/yellow/green quality threshold is applied.
...
| TPS / TTFT / Cache / thinking / goal / swarm | incremental parsing of `turn.prompt`, `step.end`, `llm.request`, `turn.cancel`, `config.update`, `goal.*` and `swarm_mode.*` events in **all** `~/.kimi-code/sessions/*/session_<id>/agents/*/wire.jsonl` of the session (main + every subagent; legacy `ses_<id>` also supported). Samples carry the event timestamp and are bucketed per agent: only the freshest 5 within the last 10 minutes feed each agent's median, so resume continuations, long idle gaps and compactions never mix in stale numbers. With several agents active at once (swarm/subagent runs — a sample within 2 minutes or a request in flight) the segment shows the **fleet total plus head count and per-agent average** (`⚡ 305 t/s (12 agents @25)`), and TTFT is the median across active agents so one stuck agent cannot poison the display. While a turn is running (from `turn.prompt` until `end_turn`/`turn.cancel`) the segment swaps TTFT for a live `gen Ns` timer that spans tool calls and steps — how long the command has been working, not just one request. Cache, thinking, goal and swarm read the main agent's wire only (per-agent byte offsets and current-turn Cache counters persist in `~/.kimi-code-hud/metrics-<sessionId>.json`; only new bytes are read each second, with a one-time Cache restoration capped at 1 MiB for old state files) |
...
**No Cache segment?** A new user turn clears the previous value and waits for its first valid `step.end`. If that turn has incomplete usage fields, or an upgrade-time bounded restoration cannot find a complete `turn.prompt`, the segment stays omitted rather than showing partial statistics; the next user message starts fresh.
