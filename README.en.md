...
**Which layouts show the Context segment?** All of them: compact/normal show a usage-colored `ctx` bar, full adds the percentage and exact token counts. The host-drawn line 2 (`context: N% (tokens/max)`) can never be taken over by a plugin — that's why compact/normal keep only the bar on line 1 instead of repeating the percentage.
