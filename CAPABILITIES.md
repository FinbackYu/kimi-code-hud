...
- Release status (2026-09-23): upstream 2.1.0 shipped with PRs #3964 and
  #3970. The audited HUD contract baseline remains 2.0.2; the Event2 link
  below is still pinned to the #3970 merge for prep evidence. Audit the
  2.0.2→2.1.0 range and re-pin to the release tag before claiming coverage.
...
- [Event2 persisted record manifest (not the complete wire journal)](https://github.com/MoonshotAI/kimi-code/blob/895e9d9b868cf9899e444784130fa1946c6cef6c/packages/agent-core-v2/docs/wire-manifest.d.ts) — prep pin at PR #3970, not a 2.1.0 release-baseline pin
...
Per-release baseline audit history (0.32.0 → 2.0.2, plus the #3970
prep) lives in
...
| Kimi subscription quota | use a managed Kimi model with a valid Kimi Code login; refresh is automatic | reported 5h, weekly, and Monthly usage percentage, bar, and reset countdown; Monthly kimi/code breakdown when both components are known | available, automatic; fresh figures render normally, figures past the 60s TTL dim (gaining an explicit `[stale]` marker once they are more than an hour old), and anything older than one week (or fetched for a different region/credential slot) renders nothing until a refresh of the current context succeeds |
...
Since Kimi Code 0.43.0 (PR #3737, verified in 0.43.1), `wire.jsonl` may
...
#3970 (included in upstream 2.1.0, pending baseline audit) all five `subagent.spawned` / `subagent.started` / `subagent.completed`
...
| quota cache | schema-v3 cache tagged with a non-reversible `contextKey` (credential slot + endpoint + credential-content digest), plus reported `usedRatio`, `resetAt`, and `fetchedAt` for known 5h, weekly, and monthly windows; legacy payloads retain exact `used`/`limit` | percentage, bar, and reset countdown; stale-usable figures dim, gaining a `[stale]` marker past the 1h mark | ratio entries carry no absolute units; v1/v2 caches are invalidated and refreshed; unknown quota windows and booster-wallet facts are not rendered; do not relabel subscription quota as token billing or API balance; only a cache matching the config's current context is shown, and any credential-file change invalidates conservatively (see [KI-17](KNOWN_ISSUES.md#ki-17-same-slot-credential-changes-invalidate-quota-conservatively)) |
