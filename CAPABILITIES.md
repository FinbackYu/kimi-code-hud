...
| Kimi subscription quota | use a managed Kimi model with a valid Kimi Code login; refresh is automatic | short and weekly usage percentage, bar, and reset countdown | available, automatic; fresh figures render normally, stale ones dim to `[stale]` after the 60s TTL, and anything older than one week (or fetched for a different region/credential slot) renders nothing until a refresh of the current context succeeds |
