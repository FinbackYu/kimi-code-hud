...
Per-release baseline audit history (0.32.0 → 0.41.0) lives in
[docs/capabilities-archive.md](docs/capabilities-archive.md). The sections
below describe the current contract against the pinned 0.41.0 baseline; new
...
| quota cache | schema-v2 cache tagged with a non-reversible `contextKey` (credential slot + endpoint + credential-content digest), plus exact `used`, `limit`, `resetAt`, and `fetchedAt` for short and weekly windows | percentage, bar, and reset countdown; stale-usable figures dim, gaining a `[stale]` marker past the 1h mark | exact units and cache freshness are available for a detail/debug surface; do not relabel subscription quota as token billing or API balance; only a cache matching the config's current context is shown, and any credential-file change invalidates conservatively (see [KI-17](KNOWN_ISSUES.md#ki-17-same-slot-credential-changes-invalidate-quota-conservatively)) |
