...
test('turn.prompt leaves the session ratio untouched', () => {
  const state = makeState();
  applyCacheWireRow(state, stepEnd('1', usage()));
  assert.deepEqual(cacheMetricFromState(state), {
    hitRate: 0.6,
  ...
  // A new prompt is not a usage event; the session-cumulative ratio simply
  // keeps showing the latest complete totals, without any freshness flag.
  applyCacheWireRow(state, prompt());
  assert.deepEqual(cacheMetricFromState(state), {
    hitRate: 0.6,
    readTokens: 300,
