...
5. **门禁**：先运行 `KIMI_HUD_RELEASE_CHECK=1 node --test test/release-metadata.test.mjs`，再确认 `npm test` 全绿 + `git diff --check`；并在 kimi-ecosystem 工作区根运行 `node scripts/check-contracts.mjs`。HUD 门禁会验证 behavior commit 可达对应 release tag；新版本 tag 尚未创建时以当前 HEAD 作为 release candidate，并在第 9 步 tag 创建后重新跑同一检查、落定到精确 tag。根门禁会验证两份文档中的 Kimi Code baseline 等于已固定的官方 annotated stable tag / peeled commit；严格 metadata 检查还会阻止两份文档版本不一致、HUD behavior baseline 未前移或 closed issue 区块仍写 working-tree / pending-release（含连字符和跨行写法）。
6. **提交**：`chore: release vX.Y.Z`，只含发布元数据与产物：`package.json`、`kimi.plugin.json`、`CHANGELOG.md`、`docs/media/*.png`，以及本次需要收口发布状态时的 `CAPABILITIES.md` / `KNOWN_ISSUES.md`。
